...
func handleBuildLevel(s *discordgo.Session, i *discordgo.InteractionCreate, data *discordgo.ApplicationCommandInteractionData) {
	option := data.Options[0]
	if option.Name != "image" || option.Type != discordgo.ApplicationCommandOptionAttachment {
		s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
			Type: discordgo.InteractionResponseChannelMessageWithSource,
	...
	req, err := http.NewRequest("POST", ServerUrl+"/api/build-level", &buff)
	if err != nil {
		errorRespond(fmt.Sprintf("Couldn't make request to server with image at '%s'", attachmentUrl), "Couldn't send the image to the server.")
		return
	}
	...
	s.FollowupMessageCreate(i.Interaction, false, &discordgo.WebhookParams{
		Content: fmt.Sprintf("File %s was successfully uploaded and the level was built.", attachment.Filename),
	})
}
...
var commands = []*discordgo.ApplicationCommand{
	...
	{
		Name:        "build-level",
		Description: "Try out building a level from an image",
		Options: []*discordgo.ApplicationCommandOption{
			{
				Name:        "image",
				Description: "Contents of the message",
